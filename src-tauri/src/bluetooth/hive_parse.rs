...
/// Registry paths for BTHPORT keys (classic + some LE), with case variants.
const BTHPORT_KEY_PATHS: &[&str] = &[
...
/// Registry paths for BTHLE keys (pure-LE devices).
const BTHLE_KEY_PATHS: &[&str] = &[
...
/// Registry paths for the device property cache.
const DEVICE_CACHE_PATHS: &[&str] = &[
...
type DeviceKeyPair = (Option<BluetoothLinkKey>, Option<BluetoothLowEnergyKey>);

/// Helper to extract typed values from a registry key's value list.
struct RegValues<'a> {
    values: &'a [KeyValue],
}

impl<'a> RegValues<'a> {
    fn new(values: &'a [KeyValue]) -> Self {
        Self { values }
    }

    fn binary_hex(&self, name: &str) -> Option<String> {
        self.values
            .iter()
            .find(|x| x.name() == name)
    ...
    fn qword_string(&self, name: &str) -> Option<String> {
        self.values
            .iter()
            .find(|x| x.name() == name)
            .and_then(|x| match x.value() {
...
pub async fn extract_hive_data(
    ...
    let bluetooth_data = task::spawn_blocking(
        move || -> std::result::Result<BluetoothData, Box<dyn std::error::Error + Send + Sync>> {
            ...
            let mut controller_devices: HashMap<String, HashMap<String, DeviceKeyPair>> = HashMap::new();

            // Scan BTHPORT (Classic + some LE devices)
            for key_path in BTHPORT_KEY_PATHS {
            ...
            for key_path in BTHLE_KEY_PATHS {
                if let Ok(Some(key)) = root_key.subpath(*key_path, &mut hive) {
...
fn parse_controller_devices(
    ...
    let mut device_map: HashMap<String, DeviceKeyPair> = HashMap::new();
    ...
    for device_connected in (*controller_key.subkeys(hive)
        ...
        let le_data = parse_le_values(&reg);
...
fn parse_le_values(reg: &RegValues) -> Option<BluetoothLowEnergyKey> {
    let ltk_key = reg.binary_hex("LTK");
    let long_term_key = ltk_key.map(|key| LongTermKeyData {
        key,
        authenticated: reg.dword("Authenticated").map(|v| v != 0),
        key_length: reg.dword("KeyLength"),
        ediv: reg.dword("EDIV"),
        rand: reg.qword_string("ERand"),
    });
    let local_csrk = reg.binary_hex("CSRK").map(|key| SignatureKeyData {
        key,
        counter: None,
        authenticated: reg.dword("MasterCSRKStatus").map(|v| v != 0),
    });
    let remote_csrk = reg.binary_hex("CSRKInbound").map(|key| SignatureKeyData {
        key,
        counter: None,
        authenticated: None,
    });
    let irk = reg.binary_hex("IRK");
...
/// Find a device node in the BTHPORT device cache and call a callback with its values.
fn find_device_in_cache<T>(
    root_key: &KeyNode,
    mac_address: &str,
    hive: &mut Hive<File, CleanHive>,
    extract: impl Fn(&[KeyValue]) -> Option<T>,
) -> Option<T> {
    for cache_path in DEVICE_CACHE_PATHS {
            ...
            if let Ok(device_subkeys) = devices_key.borrow().subkeys(hive) {
                for device_subkey in device_subkeys.iter() {
                    ...
                    if device_key.name().to_lowercase() == mac_address.to_lowercase() {
                        if let Some(result) = extract(device_key.values()) {
                            return Some(result);
                        }
                    }
                }
...
fn get_device_le_address_type(
    ...
    find_device_in_cache(root_key, mac_address, hive, |values| {
        for value in values {
